...
const RIDGE_NAMES: readonly string[] = [
  'the mouth',
  'the lip',
  'the band of cold',
  'the fold',
...
const RIDGE_SAY: readonly string[] = [
...
export function ridgeBandForClimbed(climbedPx: number): number {
  if (climbedPx < 0) return 0;
  return Math.floor(climbedPx / RIDGE_STEP_PX);
}

export function ridgeNameForBand(band: number): string {
  if (band < 0) return RIDGE_NAMES[0]!;
  return RIDGE_NAMES[band % RIDGE_NAMES.length]!;
}

export function ridgeSayForBand(band: number): string {
  if (band <= 0) return '';
...
export const RIDGE_TINTS: readonly number[] = [
...
export function worldTintForBand(band: number): number {
  if (band < 0) return 0xffffff;
