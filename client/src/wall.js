...
export class Wall {
    ...
    constructor(physics, worldWidth) {
        ...
        this.groundBody = Matter.Bodies.rectangle(w / 2, START_Y + 20, w, 40, {
            isStatic: true,
            label: 'ground',
        ...
        const leftWall = Matter.Bodies.rectangle(-100, 0, 200, wallSpan, { isStatic: true, label: 'left-wall' });
        const rightWall = Matter.Bodies.rectangle(w + 100, 0, 200, wallSpan, { isStatic: true, label: 'right-wall' });
        // Safety net far below the ledge in case of any weirdness.
        const catchFloor = Matter.Bodies.rectangle(w / 2, START_Y + 600, w * 2, 100, { isStatic: true, label: 'catch-floor' });
    ...
    maybeAppendInfiniteClimb(climberY) {
    ...
    trollSpawnChance() {
        ...
        return Math.min(TROLL_SPAWN_MAX, 0.055 + this.proceduralChunkCount * 0.0038);
    }
    /** When we already know this row is a troll, pick a kind. */
    pickTrollKindNemesisOrRandom() {
        if (Math.random() < 0.18) {
        ...
        return ALL_TROLL_IDS[(Math.random() * ALL_TROLL_IDS.length) | 0];
    }
    pickProceduralTroll() {
        if (Math.random() > this.trollSpawnChance())
            return null;
        return this.pickTrollKindNemesisOrRandom();
    }
    appendProceduralChunk() {
        if (this.appendInProgress)
            return;
     ...
     */
    syncProceduralTopYFromHolds() {
        let minY = null;
        for (const h of this.holds) {
            if (h.broken)
                continue;
            if (!h.id.startsWith('p'))
                continue;
    ...
    addWallTextureDownToY(newYMin) {
        if (newYMin >= this.voidWallYTop)
            return;
