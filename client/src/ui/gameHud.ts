...
let rafHudLoop: { alive: boolean; handle: number } = { alive: false, handle: 0 };

function readBoolKey(key: string, defaultVal: boolean) {
  const v = localStorage.getItem(key);
  if (v === null) return defaultVal;
  return v === '1';
}

export function mountGameHud(game: Game) {
  ...
  const togH = document.getElementById('fodderTogHeight')! as HTMLInputElement;
  const togS = document.getElementById('fodderTogSound')! as HTMLInputElement;
  ...
  document.getElementById('fodderToLedge')!.addEventListener('click', () => {
    if (
      !window.confirm(
        'Return to the starting ledge? This clears the shared fall marks (the dots) for this tower for everyone, and restarts your climb from here.'
      )
    ) {
      return;
    }
