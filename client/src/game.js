...
import { playPlatformBonk, setAmbienceVolume, setFallWhooshState, startAmbienceOnUserIntent, } from './audio/ambience';
...
import { ridgeBandForClimbed, ridgeNameForBand, ridgeSayForBand, worldTintForBand, } from './ridges';
...
function worldWidthFromApp(app) {
...
export class Game {
    ...
    camera = { x: 0, y: 0 };
    lastAirborne = false;
    highestPoint = START_Y;
    /** Best height this "visit" to the run — for ridge / tint; not reset on a fall. */
    sessionWallBestY = START_Y;
    lastNarratedRidgeBand = -1;
    firstFallSaid = false;
    idleOnGroundMs = 0;
    IDLE_RESET_THRESHOLD_MS = 1200;
    shakeT = 0;
    shakeMag = 0;
    shakeX = 0;
    shakeY = 0;
    ...
    extremeFall = false;
    lastWhooshOn = false;
    trollNarrationSpoken = new Set();
    trollTimerByHold = new Map();
    paused = false;
    rebuildLock = false;
    cullN = 0;
    tickerHandler;
    ...
    gameFrame(deltaMs) {
        if (this.rebuildLock)
            return;
        if (this.paused) {
            this.narrator.update(performance.now(), this.app.screen.width, this.app.screen.height);
            return;
        }
        this.tick(deltaMs);
    }
    ...
    addTimer(holdId, delayMs, onFire) {
    ...
    clearTrollTimersForHold(holdId) {
        const s = this.trollTimerByHold.get(holdId);
        if (!s)
            return;
        for (const t of s)
    ...
    speakTrollTypeOnce(t, delayMs = 220) {
        if (this.trollNarrationSpoken.has(t))
            return;
        ...
        if (troll === 'break_immediate') {
            this.addTimer(id, TROLL.breakImmediateDelayMs, () => {
                ...
                if (id === MAIN_LIE_ID) {
                    ...
                    setTimeout(() => this.narrator.say(trollNarration.break_immediate), 400);
                    setTimeout(() => {
                        this.wall.revealAllHidden();
                        setTimeout(() => this.narrator.say('here. this path was here.'), 1600);
                    }, 1200);
                }
                else {
                    this.runBreakHold(id, 'break_immediate', () => this.speakTrollTypeOnce('break_immediate', 0));
                }
            });
        }
        if (troll === 'break_after_short') {
            this.addTimer(id, TROLL.breakAfterShortMs, () => {
                this.runBreakHold(id, 'break_after_short', () => this.speakTrollTypeOnce('break_after_short', 0));
            });
        }
        if (troll === 'rot_stone') {
            this.addTimer(id, TROLL.rotStoneMs, () => {
                this.runBreakHold(id, 'rot_stone', () => this.speakTrollTypeOnce('rot_stone', 0));
            });
        }
        if (troll === 'fickle') {
    ...
    applyExtremeFall() {
        if (!this.extremeFall || !this.climber.isFree)
            return;
    ...
    trollCreepKinematics(deltaMs) {
        ...
        for (const h of this.climber.trollKinematicsGripped(CREEPY_TROLLS)) {
            ...
            if (h.troll === 'creep_down') {
                ...
                this.wall.nudgeHoldPos(h.id, h.driftDir * TROLL.driftLeanPerSec * t, 0);
            }
        }
    ...
    tick(deltaMs) {
        ...
        this.cullN += 1;
        if (this.cullN % 10 === 0) {
            this.wall.cullDistantHolds(this.climber.pos.y, this.climber.grippingHoldIds());
        }
        this.narrator.update(performance.now(), this.app.screen.width, this.app.screen.height);
    }
    handleInput() {
    ...
    detectProgressAndFalls(deltaMs) {
        const y = this.climber.pos.y;
        ...
        const whooshOn = this.isSoundWanted() &&
            !this.paused &&
            this.climber.isFree &&
            !onGround;
    ...
    bumpOnLanding(impact) {
        this.shakeT = 160;
        this.shakeMag = Math.min(8, 1.6 + impact * 0.28);
    }
    recordFall() {
        this.memory.recordFall({ x: this.climber.pos.x, y: this.climber.pos.y }, pickFallWhisper());
