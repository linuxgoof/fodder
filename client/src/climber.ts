...
import type { HandId, Hold, TrollId, Vec2 } from './types';
...
export class Climber {
  ...
  private wall: Wall;
  private hands: Record<HandId, HandState> = {
    left: { grippedHoldId: null, constraint: null },
    right: { grippedHoldId: null, constraint: null },
  };

  private torsoGfx: Graphics;
  private handsGfx: Graphics;
  private reachGfx: Graphics;
  private highlightGfx: Graphics;

  constructor(physics: Physics, wall: Wall, startPos: Vec2) {
  ...
  private reachOriginFor(hand: HandId, target: Vec2): Vec2 {
    const other: HandId = hand === 'left' ? 'right' : 'left';
    ...
    if (d < 1) return { x: torso.x, y: torso.y };
  ...
  private canReach(hand: HandId, point: Vec2): boolean {
  ...
  holdsInReach(hand: HandId): Hold[] {
    return this.wall.holds.filter(
      (h) => !h.broken && !h.hidden && this.canReach(hand, h.pos)
    );
  }
  ...
  tryGrip(hand: HandId, target: Vec2): Hold | null {
    this.release(hand);

    const hold = this.wall.nearestHold(target, 40);
    if (!hold) return null;
    if (!this.canReach(hand, hold.pos)) return null;
    ...
    const dist = Math.hypot(
      hold.pos.x - shoulderWorldX,
      hold.pos.y - shoulderWorldY
    );
   ...
   * currently gripped.
   */
  trollKinematicsGripped(allowed: readonly TrollId[]): Hold[] {
  ...
  applySwingForce(mouseWorld: Vec2) {
    if (!this.hasAnyGrip()) return;
  ...
  handleHoldBroken(holdId: string) {
  ...
  render(activeHand: HandId, mouseWorld: Vec2) {
    const p = this.body.position;
